...
![方法1的push操作](225_stack_using_queues_pushA.png)

![方法1的pop操作](225_stack_using_queues_popA.png)
...
![方法2的push操作](225_stack_using_queues_pushB.png)

![方法2的pop操作](225_stack_using_queues_popB.png)

另一个问题是，还有`top()`和`pop()`两种操作需要实现，第一种做法的`top()`操作也是`O(n)`的（因为栈顶元素在队尾，所以不得不全都重新移动一遍），所以可能第二种做法比较好。

### 方法3：只使用一个队列

一种更简单的方法是只用一个队列，`push(x)`的时候，先把`x`放到队尾，然后把`x`前面的元素全都弹出，依次插入到`x`后面；`pop()`的时候，直接弹出队头。这种方法的复杂度和方法2是相同的。[^solution]

![方法3的push操作](225_stack_using_queues_pushC.png)
