...
求出$T(m)$之后回代……总的时间复杂度为$O(m^2)$……

代码施工中……

### 70分解法2：拉格朗日插值法求等幂和

拉格朗日插值法是求解较高幂次等幂和的另一种较快的方法。
...
记中间的矩阵为$A'$，则我们找到了一种计算$T(i)$的矩阵快速幂方法。

以及，事实上，$t(i, k) = a^k k^i$在$k=0$时，当且仅当$i$也为0时才不是0，所以$\vec{t'}(0) = [1, 0, \cdots, 0, 1]^T$，$T(i)$即为${A'}^n$的右下角元素。
...
然后即可用矩阵快速幂的方法进行求解。复杂度为$O(m^3\log{n})$。
